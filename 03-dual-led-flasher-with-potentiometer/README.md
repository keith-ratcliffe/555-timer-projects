...
* ` ** ` Standard non-polarized resistor. That is, it dosen't matter which direction current
  flows through the resistors, so it won't matter how you connect them

* **NOTE:** Our capacitor, LEDs, and rectifier diodes *are* polarized, so you'll need to identify their positive (+)
...
## Step 7: Connect 100μF Capacitor to 555's TRIGGER [2] and Ground Rail (-)
   ...
   1. Connect the capacitor's positive pole (long leg, "anode") to the 555's **TRIGGER** pin **[2]**
   
   2. Connect the capacitor's negative pole (short leg, "cathode") to the breadboard's blue ground **(-)** rail
...
## Step 8: Connect 1K Ohm Resistor to 555's DISCHARGE [7] and Power Rail (+)
   ...
   1. Connect one end of the 1K ohm resistor to the 555's **DISCHARGE** pin **[7]**

   2. Connect the other end to the breadboard's right-side power **(+)** rail
...
## Step 9: Connect the First 270 Ohm Resistor (1 of 2) to 555's OUTPUT [3]
   ...
   1. Connect one end of the **270 ohm resistor** to the 555's **OUTPUT** pin **[3]**

   2. Connect the other end to an **empty row** that is 5 or more rows below the 555 chip
...
## Step 10: Connect the First Rectifier Diode (1 of 2) to 270 Ohm Resistor #1
   ...
   1. Look closely at the **1N4007 rectifier diode**. Note that its negative **(-)** pole (cathode) is denoted
      by a silver stripe on the rectifier's central barrel. Thus, its other pole is positive **(+)** (anode)

   2. Connect the positive **(+)** pole of the rectifier to the same row as your **270 ohm resistor** terminating
      row from Step 9.

   3. Connect the negative **(-)**, silver-striped pole to an empty row down below.
...
## Step 11: Connect the First LED (1 of 2) to Rectifier Diode #1 and to Ground Rail (-)
   ...
   1. Connect the LED's positive, long leg (anode) to the terminating row of rectifier #1 from Step 10.

   2. Connect the LED's negative, short leg (cathode) to the breadboard's blue ground **(-)** rail.
...
## Step 12: Connect the Second 270 Ohm Resistor (2 of 2) to 555's OUTPUT [3]
   ...
   1. Similar to Step 9, connect one end of the **270 ohm resistor** to the 555's **OUTPUT** pin **[3]**

   2. As before, connect the other end of the resistor to an **empty row** below the 555 chip.
...
## Step 13: Connect the Second Rectifier Diode (2 of 2) to 270 Ohm Resistor #2
   ...
   1. Similar to Step 10, we need to connect **Rectifier #2** to **270 Ohm Resistor #2**, but this time we must
      connect the silver-striped negative **(-)** pole of the rectifier to resistor #2's terminating row

   2. Connect the positive **(+)** pole of the rectifier to an empty row down below, but this time connect
...
## Step 14: Connect the Second LED (2 of 2) to Rectifier Diode #2 and to Power Rail (+)
   ...
   1. Connect the LED's negative, short leg (cathode) to the terminating row of rectifier #2 from Step 13.

   2. Connect the LED's positive, long leg (anode) to the red power **(+)** rail on the right side of the
      breadboard
...
## Step 15: Add Jumper Wires in Preparation for the 10K Potentiometer
  ...
  1. Use a long jumper wire to connect the 555's **DISCHARGE** pin **[7]** to an empty row at least 10 rows below the 555 chip
  2. Use a long jumper wire to connect the 555's **THRESHOLD** pin **[6]** to an empty row that is two rows ABOVE the jumper from the previous step (closer to the 555)
  3. Use another long jumper wire to connect the 555's **THRESHOLD** pin **[6]** with an empty row that is two rows ABOVE the jumper from the previous step
